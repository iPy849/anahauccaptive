...
const toggleDropdownIcon = () => { isDropdownOpen.value = !isDropdownOpen.value };
</script>
<template>
    <header class="w-100 py-2 bg-primary bg-cover bg-no-repeat bg-center grid place-content-center">
        ...
        <div id="BuildingSelector"
            class="z-50 hidden bg-white dark:bg-black border-[1px] border-primary dark:border-white rounded-lg shadow overflow-hidden w-52">
            <ul class="text-sm text-gray-700 dark:text-gray-200" aria-labelledby="BuildingSelectorButton">
                <li v-for="(location, index) in locations" :key="index"
                    :class="{ 'bg-primary': currentLocationIndex === index }">
                    <p @click="currentLocationIndex = index" :class="{ 'text-white': currentLocationIndex === index }"
                        class="block px-4 py-2 text-black dark:text-white">{{ location
                        }}
                    </p>
                </li>
            </ul>
